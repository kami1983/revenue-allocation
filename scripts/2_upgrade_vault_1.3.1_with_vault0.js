...
async function main() {
  ...
  if (old_vault_address != vault_proxy_address_of_sid_0) {
    console.log('old_vault_address != vault_proxy_address_of_sid_0');
    return;
  }

  // Check vault contract balance.
  console.log('will upgrade vault_address - ', vault_proxy_address_of_sid_0)

  // getDividendAddress()
  const vault_proxy = await ethers.getContractAt("EcoVault", vault_proxy_address_of_sid_0);

  // Get vault balance.
  const vault_balance = await ethers.provider.getBalance(vault_proxy_address_of_sid_0);
  ...
  const new_vault = await upgrades.upgradeProxy(vault_proxy_address_of_sid_0, EcoVault, { initializer: 'initialize_131', args: [] });
